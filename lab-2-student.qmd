...
ggplot(surveys, aes(x = weight, y = hindfoot_length)) +
  geom_point(alpha = 0.3) +  # Adding transparency
  facet_wrap(~ species_id) +  # Faceting by species
  labs(
    title = "Scatterplot of Weight vs. Hindfoot Length by Species",
    x = "Weight (g)", 
    y = "",  # Empty y-axis label
    subtitle = "Y-axis: Hindfoot Length (mm)"  # Move y-axis label to subtitle
  ) +
  theme(
...
ggplot(data = surveys, mapping = aes(x = species, y = weight)) +
  geom_boxplot(outlier.shape = NA) +  # Rm outliers
  geom_jitter(width = 0.2, color = "steelblue", alpha = 0.5) +  # jittered points w/ color/transparency
  labs(
    title = "Distribution of Weight by Rodent Species",
    x = "Rodent Species",
    y = "Weight (g)"  # y-axis label
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),  # Rotating species names
    axis.title.x = element_text(margin = margin(t = 10)),  # Adj x-axis margin
    axis.title.y = element_text(margin = margin(r = 10))  # Adjust y-axis margin
  )
...
The F-value is 19916, which is extremely high, and the p-value is reported as < 2e-16, which is significantly smaller than the typical alpha level of 0.05.
Given this very small p-value, we reject the null hypothesis which stated that the population mean weight is the same between all fourteen rodent species. Instead, we have evidence to support the alternative hypothesis suggesting that at least one rodent species has a different population mean weight.

So, the ANOVA results indicate a statistically significant difference in the mean weights between the rodent species.

Revised :
Based on the F-test, we reject the null hypothesis, which posits that all group means are equal. The p-value (< 2e-16) is far below the significance level (α = 0.05), providing strong evidence that at least one rodent species has a different mean weight compared to the others.

I see now I didnt fuly complete my conclusion. But now i undestand what it is I found from anova test. For future will make sure I write exactly what it is I concluded. 
...
For question 10  I used species id instead of just species. I need to be more careful when
reading what variables I am using in my code. So in future labs I will be paying more attention. 
