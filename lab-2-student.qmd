...
ggplot(data = surveys, 
  ...
  facet_wrap(~ species_id) +  # Facetng by species
  labs(
    title = "Relationship Between Weight and Hindfoot Length by Species",
    subtitle = "Hindfoot Length (mm)",  # Moving the y-axis label into the subtitle
    x = "Weight (g)",
    y = ""  
  ) +
  theme(
    plot.title = element_text(hjust = 0.5),  # Centering 
    plot.subtitle = element_text(hjust = 0.5, margin = margin(b = 10)),  # Adjusting subtitle position
    axis.title.x = element_text(margin = margin(t = 10)),  # Adding space to x-axis label
    axis.title.y = element_blank()  # Removing y label
  )
```
